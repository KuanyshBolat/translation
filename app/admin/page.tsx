...
import { Lock, Save, Plus, Trash2, Eye, EyeOff } from "lucide-react"
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      ...
      if (savedModerators) setModerators(JSON.parse(savedModerators))
      if (savedSchedule) setSchedule(JSON.parse(savedSchedule))
      if (savedVideos) setVideoConfig(JSON.parse(savedVideos))
    }
  }, [isAuthenticated])
  ...
  const saveData = () => {
    localStorage.setItem("itfest_speakers", JSON.stringify(speakers))
    localStorage.setItem("itfest_moderators", JSON.stringify(moderators))
    localStorage.setItem("itfest_schedule", JSON.stringify(schedule))
    localStorage.setItem("itfest_videos", JSON.stringify(videoConfig))
          ...
          <Card className="p-6 bg-slate-900/80 backdrop-blur-sm border-slate-700">
            ...
            <div className="space-y-4">
              {speakers.map((speaker) => (
                <div key={speaker.id} className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 bg-slate-800 rounded-lg">
                  <Input
                    placeholder="Имя"
                    value={speaker.name}
          ...
          <Card className="p-6 bg-slate-900/80 backdrop-blur-sm border-slate-700">
            ...
            <div className="space-y-4">
              {moderators.map((moderator) => (
                <div key={moderator.id} className="grid grid-cols-1 md:grid-cols-5 gap-3 p-4 bg-slate-800 rounded-lg">
                  <Input
                    placeholder="Имя"
                  ...
                  <Input
                    ...
                    onChange={(e) => updateModerator(moderator.id, "image", e.target.value)}
                    className="bg-slate-700 border-slate-600 text-white"
                  />
                  <Button
              ...
              {schedule.map((event) => (
                <div key={event.id} className="grid grid-cols-1 md:grid-cols-6 gap-3 p-4 bg-slate-800 rounded-lg">
                  ...
                  <Input
                    ...
                    className="bg-slate-700 border-slate-600 text-white md:col-span-3"
                  />
                  <Input
